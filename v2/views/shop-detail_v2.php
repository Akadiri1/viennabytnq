...
// Page meta for v2 header
$v2_page_title = htmlspecialchars($singleProduct['name']) . ' - ' . $site_name;
$v2_page_id = 'product';
$v2_header_transparent = false;
include APP_PATH."/views/includes/header_v2.php";
?>
<!-- V1 Tailwind + Libraries (needed for product detail layout) -->
<script src="https://cdn.tailwindcss.com/3.4.1"></script>
<script>
    tailwind.config = { theme: { extend: { colors: { "brand-bg": "#FFFFFF", "brand-text": "#1A1A1A", "brand-gray": "#6B6B6B", "brand-red": "#8B0000", }, fontFamily: { sans: ["Inter", "ui-sans-serif", "system-ui"], serif: ["Playfair Display", "serif"], }, }, }, };
</script>
<script src="https://unpkg.com/feather-icons"></script>
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/toastify-js"></script>
<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/toastify-js/src/toastify.min.css">

<!-- V2 Design Overrides for Shop Detail -->
<style>
  /* Base overrides to match v2 aesthetic */
  body.v2 { background-color: #fff !important; font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif !important; }

  /* Breadcrumb hero - cleaner, shorter */
  .v2 section.relative.h-64 { height: 220px !important; }
  .v2 section.relative.h-64 h1 { font-family: 'Inter', sans-serif !important; font-size: 2rem !important; font-weight: 500 !important; letter-spacing: 0.06em !important; text-transform: uppercase !important; }
  .v2 section.relative.h-64 nav { font-family: 'Inter', sans-serif !important; font-size: 0.6875rem !important; letter-spacing: 0.1em !important; text-transform: uppercase !important; }

  /* Main content area */
  .v2 main.container { padding-top: 2.5rem !important; padding-bottom: 3rem !important; max-width: 1400px !important; }

  /* Product title */
  .v2 h1.sr-only + .flex p#product-price,
  .v2 #product-price { font-family: 'Inter', sans-serif !important; font-weight: 400 !important; font-size: 1.25rem !important; }
  .v2 .price-from-label { font-size: 0.6em !important; }

  /* Dropdowns - v2 clean style */
  .v2 .custom-dropdown-trigger {
  ...
  .v2 .custom-dropdown-trigger:hover { border-color: #1A1A1A !important; }
  .v2 .custom-dropdown-menu {
    border: 1px solid #E5E5E5 !important;
    border-radius: 0 !important;
  ...
  .v2 .custom-option:hover { background: #F7F7F5 !important; }

  /* Section labels */
  .v2 h3.text-xs { font-family: 'Inter', sans-serif !important; font-size: 0.6875rem !important; letter-spacing: 0.15em !important; font-weight: 500 !important; }
  ...
  .v2 #add-to-cart-btn:hover { background: #333 !important; }
  .v2 #add-to-cart-btn:disabled { opacity: 0.5 !important; }

  /* Quantity controls */
  .v2 #quantity-minus, .v2 #quantity-plus { font-family: 'Inter', sans-serif !important; }
  .v2 #quantity-display { font-family: 'Inter', sans-serif !important; font-size: 0.875rem !important; }

  /* Custom color/size toggle buttons */
  .v2 #open-custom-color-btn, .v2 #open-custom-size-btn, .v2 #open-size-chart-btn {
    font-family: 'Inter', sans-serif !important;
  ...
  .v2 .form-input-sleek:focus { border-bottom-color: #1A1A1A !important; }

  /* Thumbnails */
  .v2 #thumbnail-gallery .skeleton-container { border-radius: 0 !important; }
  .v2 .active-thumbnail { border-color: #1A1A1A !important; }

  /* Image slider */
  .v2 #main-image-slider { border-radius: 0 !important; }

  /* Stock display */
  .v2 #stock-display { font-family: 'Inter', sans-serif !important; font-size: 0.75rem !important; letter-spacing: 0.03em !important; }

  /* Product text */
  .v2 .prose { font-family: 'Inter', sans-serif !important; font-size: 0.8125rem !important; line-height: 1.7 !important; color: #6B6B6B !important; }

  /* Currency switcher v2 style */
  .v2 .currency-switcher { border-radius: 0 !important; background: transparent !important; }
  .v2 .currency-switcher a.currency-link {
    font-family: 'Inter', sans-serif !important;
    font-size: 0.6875rem !important;
    font-weight: 500 !important;
  ...
  .v2 .currency-switcher a.currency-link.active,
  .v2 .currency-switcher a.currency-link.bg-white {
    background: #1A1A1A !important;
    color: #fff !important;
  ...
  .v2 .product-card .price-display {
    font-family: 'Inter', sans-serif !important;
    font-size: 0.75rem !important;
  }

  /* Reviews section - v2 aesthetic */
  .v2 #reviews-section {
    background: #FFFFFF !important;
    border-top: 1px solid #E5E5E5 !important;
  }
  .v2 #reviews-section h2 {
    font-family: 'Inter', sans-serif !important;
  ...
  .v2 #reviews-section p.text-brand-gray {
    font-family: 'Inter', sans-serif !important;
  }
  .v2 #reviews-section .bg-white {
    border: 1px solid #E5E5E5 !important;
    border-radius: 0 !important;
  }
  .v2 #reviews-section h4 {
    font-family: 'Inter', sans-serif !important;
    font-size: 0.875rem !important;
    font-weight: 500 !important;
  }
  .v2 #reviews-section .font-lato {
    font-family: 'Inter', sans-serif !important;
    font-size: 0.8125rem !important;
  }
  .v2 #review-form textarea {
    border-radius: 0 !important;
    font-family: 'Inter', sans-serif !important;
  }
  .v2 #submit-review-btn {
    background: #1A1A1A !important;
    border-radius: 0 !important;
    font-family: 'Inter', sans-serif !important;
    font-size: 0.75rem !important;
  ...
  .v2 #submit-review-btn:hover { background: #333 !important; }

  /* Review login prompt & no-reviews */
  .v2 #reviews-section .bg-gradient-to-br,
  .v2 #reviews-section .bg-gradient-to-br.from-gray-50 {
    background: #F7F7F5 !important;
    border: 1px solid #E5E5E5 !important;
    border-radius: 0 !important;
  }
  .v2 #reviews-section a.inline-block {
    background: #1A1A1A !important;
  ...
  .v2 #reviews-section a.inline-block:hover {
    background: #333 !important;
    opacity: 1 !important;
  }
  /* Lock icon & message icon backgrounds */
  .v2 #reviews-section .inline-block.p-3.bg-white.rounded-full {
    background: #fff !important;
    border: 1px solid #E5E5E5 !important;
    border-radius: 0 !important;
  }
  /* "No reviews yet" dashed border box */
  .v2 #no-reviews-msg {
    background: #FAFAFA !important;
    border: 2px dashed #E5E5E5 !important;
    border-radius: 0 !important;
  }
  .v2 #no-reviews-msg p.font-serif {
    font-family: 'Inter', sans-serif !important;
    font-size: 1.125rem !important;
    font-weight: 500 !important;
    letter-spacing: 0.06em !important;
  ...
  .v2 #no-reviews-msg p.font-lato {
    font-family: 'Inter', sans-serif !important;
  }
  /* "Join the Conversation" card */
  .v2 #reviews-section h3.font-serif {
    font-family: 'Inter', sans-serif !important;
    font-size: 0.875rem !important;
    font-weight: 500 !important;
    letter-spacing: 0.08em !important;
  ...
  .v2 #reviews-section p.font-lato {
    font-family: 'Inter', sans-serif !important;
  }
  /* Rating summary card */
  .v2 #reviews-section .bg-white.border {
    border: 1px solid #E5E5E5 !important;
    border-radius: 0 !important;
  }
  ...
  /* Sold out badge on related products */
  .v2 .product-card span.bg-\[\#1a1a1a\] {
    font-family: 'Inter', sans-serif !important;
  ...
  /* Fix: ensure product images aspect ratio works with v2 */
  .v2 .aspect-\[9\/16\] { aspect-ratio: 9/16; }

  /* Padding top for main content to account for v2 fixed header */
  .v2 section.relative.h-64 { margin-top: 60px; }

  /* Cart sidebar fixes - ensure text visible */
  .v2 #cart-sidebar a.bg-brand-text,
  .v2 #cart-sidebar a[href="/checkout"] {
    color: #FFFFFF !important;
    background: #1A1A1A !important;
  }
  .v2 #cart-sidebar a[href="/view-cart"] {
    color: #1A1A1A !important;
  }
  .v2 #cart-sidebar .bg-brand-bg { background: #fff !important; }
  .v2 #cart-sidebar h2 { font-family: 'Inter', sans-serif !important; }

  /* Mobile adjustments */
  @media (max-width: 768px) {
    .v2 section.relative.h-64 { height: 180px !important; margin-top: 56px; }
    .v2 section.relative.h-64 h1 { font-size: 1.5rem !important; }
    .v2 main.container { padding-top: 1.5rem !important; }
  }
</style>
    <style>
        ...
        /* Star Button Interactive States */
        .star-btn {
            display: inline-flex;
            align-items: center;
        ...
        #reviews-list > div:nth-child(1) { animation-delay: 0s; }
        #reviews-list > div:nth-child(2) { animation-delay: 0.1s; }
        #reviews-list > div:nth-child(3) { animation-delay: 0.2s; }
        #reviews-list > div:nth-child(4) { animation-delay: 0.3s; }
        #reviews-list > div:nth-child(n+5) { animation-delay: 0.4s; }
        ...
        /* Review Text Truncation Support */
        .line-clamp-none {
            display: block;
        ...
        /* Avatar Enhancement */
        .rounded-full[class*="bg-gradient"] {
            background-clip: content-box;
        }
        
        /* Smooth Transitions for All Review Elements */
        #reviews-list * {
...
<!-- V1 sidebar & cart sidebar kept for JS functionality -->
    ...
    <!-- HEADER - HIDDEN (v2 header is used instead) -->
    <header class="bg-white/10 backdrop-blur-lg border-b border-gray-200/60 sticky top-0 z-40" style="display:none;"><div class="container mx-auto px-4 sm:px-6 lg:px-8"><div class="flex items-center justify-between h-16"><div class="flex-1 flex justify-start"><button id="open-sidebar-btn" class="p-2 text-brand-text hover:text-brand-gray"><i data-feather="menu" class="h-6 w-6"></i></button></div><div class="flex-shrink-0 text-center"><a href="/home"><img src="<?=$logo_directory?>" alt="<?=$site_name?>" class="h-8 w-auto object-contain"></a></div><div class="flex-1 flex items-center justify-end space-x-2 md:space-x-4">
    ...
    <!-- REVIEWS SECTION                                                     -->
    <!-- =================================================================== -->
    <section class="bg-gradient-to-b from-white via-gray-50/50 to-white py-16 md:py-28 border-t border-gray-200" id="reviews-section">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Header -->
            <div class="text-center mb-16 md:mb-20">
                <h2 class="text-4xl md:text-5xl font-serif font-semibold text-brand-text mb-3">Customer Reviews</h2>
                <p class="text-brand-gray font-lato text-sm uppercase tracking-widest font-medium">See what our valued customers are saying</p>
            </div>

            <!-- Main Grid: Responsive Layout -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                
                <!-- LEFT: Summary & Review Form (Mobile-Friendly) -->
                <div class="md:col-span-1 order-2 md:order-1">
                    
                    <!-- Rating Summary Card -->
                    <div class="bg-white border border-gray-200 rounded-lg p-8 md:p-10 shadow-sm hover:shadow-md transition-shadow mb-8">
                        <div class="text-center">
                            <div id="avg-rating-value" class="text-7xl md:text-6xl font-serif text-brand-text mb-3 font-bold"><?= $averageRating ?></div>
                            <div id="avg-rating-stars" class="flex justify-center gap-1.5 text-yellow-400 mb-4">
                                <?php for($i=1; $i<=5; $i++): ?>
                                    <i data-feather="star" class="w-5 h-5 md:w-4 md:h-4 <?= $i <= round($averageRating) ? 'fill-current' : 'text-gray-300' ?>"></i>
                                <?php endfor; ?>
                            </div>
                            <p id="reviews-count" class="text-sm font-lato text-brand-gray font-medium">Based on <span class="font-semibold text-brand-text"><?= $totalReviews ?></span> review<?= $totalReviews !== 1 ? 's' : '' ?></p>
                        </div>
                    </div>

                    <!-- Review Form (Logged In Only) -->
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <div class="bg-white border border-gray-200 rounded-lg p-8 md:p-10 shadow-sm">
                            <h3 class="text-2xl font-serif text-brand-text mb-6 font-semibold">Share Your Review</h3>
                            <form id="review-form" class="space-y-6">
                                <input type="hidden" name="product_id" value="<?= $id ?>">
                                
                                <!-- Star Input -->
                                <div>
                                    <label class="block text-xs uppercase tracking-widest font-bold text-brand-text mb-4">Select Rating</label>
                                    <div class="flex gap-3" id="star-rating-input">
                                        <?php for($i=1; $i<=5; $i++): ?>
                                            <button type="button" class="star-btn group text-gray-300 hover:text-yellow-400 transition-all transform hover:scale-110 duration-200" data-value="<?= $i ?>" title="Rate <?= $i ?> stars">
                                                <i data-feather="star" class="w-8 h-8 md:w-7 md:h-7 fill-current group-hover:drop-shadow-lg"></i>
                                            </button>
                                        <?php endfor; ?>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="review_text" rows="5" class="w-full bg-white border border-gray-200 rounded-lg p-4 text-sm font-lato focus:outline-none focus:ring-2 focus:ring-brand-text focus:ring-opacity-50 focus:border-transparent resize-none transition-all placeholder-gray-400" placeholder="Tell us about your experience with this piece..." required></textarea>
                                    <p class="text-xs text-brand-gray mt-2 font-lato">Be honest and helpful to other customers</p>
                                </div>

                                <!-- Submit Button -->
                                <button type="submit" class="w-full bg-brand-text text-white py-3 md:py-4 font-semibold rounded-lg hover:bg-gray-800 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider text-sm font-medium shadow-md hover:shadow-lg" id="submit-review-btn">Submit Review</button>
                            </form>
                        </div>
                    <?php else: ?>
                        <div class="bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 rounded-lg p-8 md:p-10 text-center shadow-sm hover:shadow-md transition-shadow">
                            <div class="mb-4 inline-block p-3 bg-white rounded-full">
                                <i data-feather="lock" class="w-6 h-6 text-brand-text"></i>
                            </div>
                            <h3 class="font-serif text-lg md:text-xl text-brand-text mb-3 font-semibold">Join the Conversation</h3>
                            <p class="text-sm font-lato text-brand-gray mb-6 leading-relaxed">Create an account or log in to share your review and help other customers.</p>
                            <a href="/register?form=login&redirect=<?= urlencode($_SERVER['REQUEST_URI']) ?>" class="inline-block bg-brand-text text-white px-8 py-3 rounded-lg font-medium hover:bg-gray-800 transition-all duration-200 text-sm tracking-wider uppercase shadow-md hover:shadow-lg font-semibold">Log In to Review</a>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="md:col-span-2 order-1 md:order-2">
                    <div id="reviews-list" class="space-y-6 md:space-y-8">
                        <?php if ($totalReviews > 0): ?>
                            <?php foreach ($productReviews as $review): ?>
                                <div class="bg-white border border-gray-200 rounded-lg p-6 md:p-8 hover:shadow-md transition-shadow duration-300">
                                    <!-- Review Header -->
                                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5 pb-4 border-b border-gray-100">
                                        <div class="flex items-start gap-4 flex-1">
                                            <!-- Avatar -->
                                            <div class="flex-shrink-0">
                                                <div class="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-brand-text/10 to-brand-text/20 flex items-center justify-center text-brand-text font-serif text-xl md:text-2xl font-bold border border-brand-text/20 shadow-sm">
                                                    <?= strtoupper(substr($review['reviewer_name'], 0, 1)) ?>
                                                </div>
                                            </div>
                                            <!-- Name & Meta -->
                                            <div class="flex-grow min-w-0">
                                                <h4 class="font-serif font-semibold text-lg md:text-base text-brand-text truncate"><?= htmlspecialchars($review['reviewer_name']) ?></h4>
                                                <p class="text-xs font-lato text-brand-gray uppercase tracking-wider mt-1.5 font-medium">
                                                    <?= date('M j, Y', strtotime($review['created_at'])) ?>
                                                </p>
                                            </div>
                                        </div>
                                        <!-- Stars -->
                                        <div class="flex gap-1 text-yellow-400 flex-shrink-0">
                                            <?php for($i=1; $i<=5; $i++): ?>
                                                <i data-feather="star" class="w-5 h-5 md:w-4 md:h-4 <?= $i <= $review['rating'] ? 'fill-current' : 'text-gray-300' ?>"></i>
                                            <?php endfor; ?>
                                        </div>
                                    </div>
                                    
                                    <!-- Review Text -->
                                    <p class="text-sm md:text-base font-lato text-gray-700 leading-relaxed line-clamp-none"><?= nl2br(htmlspecialchars($review['review_text'])) ?></p>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <!-- No Reviews State -->
                            <div id="no-reviews-msg" class="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-dashed border-gray-200 rounded-lg py-16 md:py-20 text-center">
                                <div class="mb-4 inline-block p-3 bg-white rounded-full">
                                    <i data-feather="message-square" class="w-8 h-8 text-gray-400"></i>
                                </div>
                                <p class="font-serif text-xl md:text-2xl text-brand-text mb-2 font-semibold">No reviews yet</p>
                                <p class="font-lato text-sm md:text-base text-brand-gray px-4">Be the first to share your thoughts on this beautiful piece.</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (starBtns.length) {
            ...
            btn.addEventListener('click', () => {
                ...
                starBtns.forEach(b => {
                    ...
                    if (bVal <= value) {
                        ...
                        b.classList.add('text-yellow-400', 'active');
        ...
        document.getElementById('star-rating-input')?.addEventListener('mouseleave', () => {
            ...
            starBtns.forEach(b => {
                const bVal = parseInt(b.dataset.value);
                if (currentValue && bVal <= parseInt(currentValue)) {
                    b.classList.add('text-yellow-400');
                } else {
                    b.classList.remove('text-yellow-400');
                }
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-6 md:p-8 hover:shadow-md transition-shadow duration-300">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5 pb-4 border-b border-gray-100">
                                <div class="flex items-start gap-4 flex-1">
                                    ...
                                    <!-- Name & Meta -->
                                    <div class="flex-grow min-w-0">
                                        <h4 class="font-serif font-semibold text-lg md:text-base text-brand-text truncate">${review.reviewer_name}</h4>
                                        <p class="text-xs font-lato text-brand-gray uppercase tracking-wider mt-1.5 font-medium">${review.created_at}</p>
                                    </div>
                                </div>
                                <!-- Stars -->
                                <div class="flex gap-1 text-yellow-400 flex-shrink-0">
                            ...
                            <!-- Review Text -->
                            <p class="text-sm md:text-base font-lato text-gray-700 leading-relaxed line-clamp-none">${review.review_text}</p>
                        </div>
                    `;
