<?php
// V2 Router - Routes to _v2.php view files where available,
// falls back to original view files for AJAX/auth/admin routes.
...
// Helper: set $_v2_view then include this snippet to load _v2.php or fallback
// We avoid a function so that $conn, $site_name, etc. remain in scope.
$_v2_include = function($viewName) {
    $v2File = APP_PATH."/views/{$viewName}_v2.php";
...
if (count($uri) > 2 && !empty($uri[2])) {
    ...
    switch ($route_key) {
        ...
        case "product/" . $uri[2]:
            include($_v2_include('shop-detail'));
            die();
            break;

        default:
            http_response_code(404);
            include($_v2_include('404'));
            die();
            break;
    }
    ...
    switch ($route) {
        // --- PUBLIC PAGES (use _v2 views) ---
        case 'home':
        case 'index':
            include($_v2_include('home'));
            break;
        ...
        case 'about':
            include($_v2_include('about'));
            break;

        case 'contact-us':
            include($_v2_include('contact'));
            break;

        case 'view-cart':
            include($_v2_include('cart'));
            break;

        case 'checkout':
            include($_v2_include('checkout'));
            break;

        case 'login':
            include($_v2_include('login'));
            break;

        case 'register':
            include($_v2_include('register'));
            break;

        case 'privacy':
            include($_v2_include('privacy'));
            break;

        case 'user-dashboard':
            include($_v2_include('user-dashboard'));
            break;

        case 'invoice':
            include($_v2_include('invoice'));
            break;

        case 'order-view':
            include($_v2_include('order-view'));
            break;
        ...
        // --- AJAX / API routes (no _v2 needed, backend logic unchanged) ---
        case 'cart':
