...
// Mock DB connection and functions if not included
if (!isset($conn) || !function_exists('selectContent')) {
    // This is a placeholder for demonstration purposes.
    // In your real environment, you would have your db_connection.php included.
    class MockPDO {
    ...
    function selectContent($conn, $table, $conditions) {
        if ($table === 'panel_products') {
            return [[
                'id' => 1, 'name' => 'Elegant Summer Dress', 'price' => 25000, 'image_one' => 'images/placeholder1.jpg', 'image_two' => 'images/placeholder2.jpg', 'product_text' => 'This is a beautiful and elegant dress, perfect for summer occasions. Made from high-quality linen.', 'collection_id' => 101, 'visibility' => 'show'
            ]];
        }
        return [];
    }
    // Mock data
    $singleProduct = selectContent($conn, "panel_products", ['visibility' => 'show', 'id' => $id])[0];
    $priceVariants = [
        ['id' => 1, 'variant_name' => 'Standard', 'price' => 25000],
        ['id' => 2, 'variant_name' => 'With Lining', 'price' => 28500],
    ];
    $displayPrice = $priceVariants[0]['price'];
    $mainImageUrl = $singleProduct['image_one'];
    $productImages = [
        ['image_path' => 'images/placeholder3.jpg', 'alt_text' => 'Side view'],
        ['image_path' => 'images/placeholder4.jpg', 'alt_text' => 'Back view'],
    ];
    $availableColors = [
        ['id' => 1, 'color_name' => 'Beige', 'hex_code' => '#F5F5DC'],
        ['id' => 2, 'color_name' => 'Forest Green', 'hex_code' => '#228B22'],
        ['id' => 3, 'color_name' => 'Burgundy', 'hex_code' => '#800020'],
    ];
    $availableSizes = []; // Empty since price variants exist in this example
    $relatedProducts = [
        ['id' => 2, 'name' => 'Chic Jumpsuit', 'price' => 32000, 'image_one' => 'images/related1.jpg', 'image_two' => 'images/related1_hover.jpg'],
        ['id' => 3, 'name' => 'Casual Blouse', 'price' => 18000, 'image_one' => 'images/related2.jpg', 'image_two' => 'images/related2_hover.jpg'],
        ['id' => 4, 'name' => 'Evening Gown', 'price' => 45000, 'image_one' => 'images/related3.jpg', 'image_two' => 'images/related3_hover.jpg'],
        ['id' => 5, 'name' => 'Linen Trousers', 'price' => 22000, 'image_one' => 'images/related4.jpg', 'image_two' => 'images/related4_hover.jpg'],
    ];
    ...
    $displayPrice = !empty($priceVariants) ? $priceVariants[0]['price'] : $singleProduct['price'];
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 xl:gap-x-16 gap-y-10">
            ...
            <div class="flex flex-col pt-4 lg:pt-0 space-y-8">
                <div>
                    ...
                    <p id="product-price" class="price-display text-2xl text-brand-gray mt-3 mb-5" data-price-ngn="<?= $displayPrice ?>" data-base-price="<?= $singleProduct['price'] ?>">₦<?= number_format($displayPrice, 2) ?></p>
                ...
                <div>
                    ...
                    <?php if (!empty($availableColors)): ?>
                        <div id="color-selector" class="flex items-center space-x-3"><?php foreach ($availableColors as $color): ?><button data-id="<?= $color['id'] ?>" data-color="<?= htmlspecialchars($color['color_name']) ?>" class="color-swatch" style="background-color: <?= htmlspecialchars($color['hex_code']) ?>"></button><?php endforeach; ?></div>
                        ...
                        <div id="custom-color-input-container" class="is-closed"><label for="custom-color" class="text-sm font-medium text-brand-gray">Custom Color</label><input type="text" id="custom-color" class="form-input-sleek mt-1" placeholder="e.g., Emerald Green"/></div>
                    <?php else: ?>
                        <div id="custom-color-input-container"><input type="text" id="custom-color" class="form-input-sleek" placeholder="e.g., Emerald Green"/></div>
                    <?php endif; ?>
                </div>
                <div>
                    <div class="flex justify-between items-center mb-3">
                        <h3 class="text-sm font-semibold">OPTION/SIZE <span class="<?= (!empty($priceVariants) || !empty($availableSizes)) ? 'text-brand-red font-semibold' : 'text-brand-gray font-normal' ?>"><?= (!empty($priceVariants) || !empty($availableSizes)) ? '(Required)' : '(Optional)' ?></span></h3>
                        <?php if(!empty($availableSizes) || !empty($priceVariants)): // Show if any standard options exist ?>
                    ...
                    <?php if (!empty($priceVariants)): ?>
                        <div id="price-variant-selector" class="flex flex-wrap gap-2">
                            <?php foreach ($priceVariants as $variant): ?>
                                <div class="relative"><input type="radio" name="price-variant" id="variant-<?= $variant['id'] ?>" value="<?= $variant['id'] ?>" class="sr-only" data-price="<?= $variant['price'] ?>"><label for="variant-<?= $variant['id'] ?>" class="price-variant-label price-display" data-price-ngn="<?= $variant['price'] ?>"><?= htmlspecialchars($variant['variant_name']) ?> - ₦<?= number_format($variant['price'], 2) ?></label></div>
                            <?php endforeach; ?>
                        </div>
                    <?php elseif (!empty($availableSizes)): ?>
                        <div id="size-selector" class="flex flex-wrap gap-2"><?php foreach ($availableSizes as $size): ?><button data-id="<?= $size['id'] ?>" class="size-btn"><?= htmlspecialchars($size['size_name']) ?></button><?php endforeach; ?></div>
                    <?php endif; ?>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const selectors = { sidebar: document.getElementById("sidebar"), openSidebarBtn: document.getElementById("open-sidebar-btn"), closeSidebarBtn: document.getElementById("close-sidebar-btn"), sidebarOverlay: document.getElementById("sidebar-overlay"), cartSidebar: document.getElementById("cart-sidebar"), openCartBtn: document.getElementById("open-cart-btn"), closeCartBtn: document.getElementById("close-cart-btn"), cartOverlay: document.getElementById("cart-overlay"), cartItemsContainer: document.getElementById("cart-items-container"), cartSubtotalEl: document.getElementById("cart-subtotal"), cartItemCountEl: document.getElementById("cart-item-count"), addToCartBtn: document.getElementById("add-to-cart-btn"), quantityMinusBtn: document.getElementById("quantity-minus"), quantityPlusBtn: document.getElementById("quantity-plus"), quantityDisplay: document.getElementById("quantity-display"), openCustomColorBtn: document.getElementById("open-custom-color-btn"), customColorContainer: document.getElementById("custom-color-input-container"), customColorInput: document.getElementById("custom-color"), mainImage: document.getElementById("main-product-image"), thumbnails: document.querySelectorAll(".thumbnail-img"), colorSwatches: document.querySelectorAll(".color-swatch"), selectedColorName: document.getElementById("selected-color-name"), sizeBtns: document.querySelectorAll(".size-btn"), saveMeasurementsBtn: document.getElementById("save-measurements-btn"), priceVariantRadios: document.querySelectorAll('input[name="price-variant"]'), productPriceEl: document.getElementById("product-price"), modals: [ { modal: document.getElementById("custom-size-modal"), openBtn: document.getElementById("open-custom-size-btn"), closeBtn: document.getElementById("close-custom-size-btn"), overlay: document.getElementById("custom-size-overlay") }, { modal: document.getElementById("size-chart-modal"), openBtn: document.getElementById("open-size-chart-btn"), closeBtn: document.getElementById("close-size-chart-btn"), overlay: document.getElementById("size-chart-overlay") } ] };
    ...
    const formatCurrency = (amount, currency = 'NGN') => { if (currency === 'USD') { return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(amount); } return new Intl.NumberFormat('en-NG', { style: 'currency', currency: 'NGN' }).format(amount).replace('NGN', '₦'); };
    const toggleSidebar = (sidebar, shouldOpen) => { if (!sidebar) return; const isMenuSidebar = sidebar.id === 'sidebar'; if (shouldOpen) { sidebar.classList.remove(isMenuSidebar ? "-translate-x-full" : "translate-x-full"); document.body.classList.add("overflow-hidden"); } else { sidebar.classList.add(isMenuSidebar ? "-translate-x-full" : "translate-x-full"); if (!document.querySelector("#sidebar:not(.-translate-x-full), #cart-sidebar:not(.translate-x-full)")) document.body.classList.remove("overflow-hidden"); } };
    ...
    const updateCartDisplay = async () => { try { const response = await fetch('/update-cart', { method: 'POST' }); if (!response.ok) throw new Error('Network response was not ok.'); const cartData = await response.json(); if (cartData && cartData.status === 'success') { const container = selectors.cartItemsContainer; container.innerHTML = ''; selectors.cartSubtotalEl.dataset.priceNgn = cartData.subtotal; if (cartData.items && cartData.items.length > 0) { cartData.items.forEach(item => { let optionsHtml = ''; if (item.color_name) optionsHtml += `${item.color_name}`; else if (item.custom_color_name) optionsHtml += `Custom: ${item.custom_color_name}`; if (item.size_name) optionsHtml += (optionsHtml ? ' / ' : '') + item.size_name; else if (item.custom_size_details && item.custom_size_details !== '{}') optionsHtml += ' / Custom Size'; const itemHtml = `<div class="flex gap-4 py-4 border-b border-gray-200 last:border-b-0"><img src="/${item.product_image}" alt="${item.product_name}" class="w-20 h-24 object-cover"><div class="flex-1 flex flex-col justify-between"><div><h4 class="font-semibold text-sm">${item.product_name}</h4><p class="text-xs text-brand-gray">${optionsHtml}</p></div><div class="flex justify-between items-center mt-2"><p class="text-sm font-medium price-display" data-price-ngn="${item.total_price}">${formatCurrency(item.total_price)}</p><p class="text-xs text-brand-gray">Qty: ${item.quantity}</p></div></div><div class="flex flex-col items-center gap-2 self-start"><button class="remove-from-cart-btn p-1 text-brand-gray hover:text-brand-red" data-cart-item-id="${item.id}"><i data-feather="trash-2" class="w-4 h-4"></i></button></div></div>`; container.insertAdjacentHTML('beforeend', itemHtml); }); } else { container.innerHTML = `<div id="empty-cart-message" class="flex flex-col items-center justify-center h-full text-center"><i data-feather="shopping-bag" class="w-16 h-16 text-gray-300 mb-4"></i><p class="text-brand-gray">Your cart is empty.</p></div>`; } const lineItemCount = cartData.items.length; selectors.cartItemCountEl.textContent = lineItemCount; selectors.cartItemCountEl.style.display = lineItemCount > 0 ? 'flex' : 'none'; feather.replace(); updateAllPrices(document.querySelector('.currency-switcher a.active')?.dataset.currency || 'NGN'); } else { throw new Error(cartData.message || 'Failed to parse cart data.'); } } catch (error) { console.error('Failed to update cart display:', error); selectors.cartItemsContainer.innerHTML = `<div class="text-center text-brand-gray">Could not load cart.</div>`; } };
    const handleDeleteFromCart = async (cartItemId) => { try { const response = await fetch('/delete-cart', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ cartItemId: cartItemId }) }); const result = await response.json(); if(response.ok && result.status === 'success') { showToast(result.message); await updateCartDisplay(); } else { showToast(result.message || 'Failed to remove item.', 'error'); } } catch (error) { console.error('Delete Error:', error); showToast('A network error occurred while deleting.', 'error'); } };
    const handleAddToCart = async () => {
        const btn = selectors.addToCartBtn; if (!btn) return;
        ...
        const customSizeDetails = Array.from(document.querySelectorAll("#custom-measurements-form input")).reduce((acc, input) => { const val = input.value.trim(); if(val) acc[input.id] = val; return acc; }, {});
        const hasCustomMeasurements = Object.keys(customSizeDetails).length > 0;
        const activeVariantRadio = document.querySelector('input[name="price-variant"]:checked');
        const activeSizeBtn = document.querySelector("#size-selector .active-size");
        if (selectors.colorSwatches.length > 0) { if (!activeColorSwatch && !customColorValue) { showToast("Please select a color or provide a custom one.", "error"); resetAddToCartButton(); return; } } else if (!customColorValue) { showToast("A custom color is required for this item.", "error"); resetAddToCartButton(); return; }
        if (customColorValue) { payload.customColor = customColorValue; } else if (activeColorSwatch) { payload.colorId = parseInt(activeColorSwatch.dataset.id); }
        if (selectors.priceVariantRadios.length > 0 || selectors.sizeBtns.length > 0) { if (!hasCustomMeasurements && !activeVariantRadio && !activeSizeBtn) { showToast("Please select an option/size or provide custom measurements.", "error"); resetAddToCartButton(); return; } } else if (!hasCustomMeasurements) { showToast("Custom measurements are required for this item.", "error"); resetAddToCartButton(); return; }
        if (hasCustomMeasurements) { payload.customSizeDetails = customSizeDetails; } else if (activeVariantRadio) { payload.priceVariantId = parseInt(activeVariantRadio.value); } else if (activeSizeBtn) { payload.sizeId = parseInt(activeSizeBtn.dataset.id); }
        try { const response = await fetch("/cart", { method: "POST", headers: { "Content-Type": "application/json", "Accept": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json(); if (response.ok && result.status === "success") { showToast("Item added to cart!"); await updateCartDisplay(); toggleSidebar(selectors.cartSidebar, true); } else { showToast(result.message || "Could not add item.", "error"); } } catch (error) { console.error("Fetch Error:", error); showToast("A network error occurred.", "error"); } finally { resetAddToCartButton(); }
    };
    const USD_RATE = <?= USD_EXCHANGE_RATE ?>;
    const INITIAL_CURRENCY = '<?= $current_currency ?>';
    const updateAllPrices = (targetCurrency) => { document.querySelectorAll('.price-display').forEach(el => { const ngnPrice = parseFloat(el.dataset.priceNgn); if (!isNaN(ngnPrice)) { let newPrice = (targetCurrency === 'USD') ? ngnPrice / USD_RATE : ngnPrice; const variantName = el.textContent.includes('-') ? el.textContent.split('-')[0].trim() : ''; if (variantName && el.tagName === 'LABEL') { el.textContent = `${variantName} - ${formatCurrency(newPrice, targetCurrency)}`; } else { el.textContent = formatCurrency(newPrice, targetCurrency); } } }); };
    ...
    if(selectors.cartItemsContainer) { selectors.cartItemsContainer.addEventListener('click', (e) => { const removeBtn = e.target.closest('.remove-from-cart-btn'); if (removeBtn) { handleDeleteFromCart(parseInt(removeBtn.dataset.cartItemId)); } }); }
    ...
    selectors.priceVariantRadios.forEach(radio => { radio.addEventListener('change', function() { if (this.checked) { if (selectors.productPriceEl) { selectors.productPriceEl.dataset.priceNgn = this.dataset.price; updateAllPrices(document.querySelector('.currency-switcher a.active')?.dataset.currency || 'NGN'); } selectors.sizeBtns.forEach(b => b.classList.remove("active-size")); document.querySelectorAll("#custom-measurements-form input").forEach(input => input.value = ''); } }); });
    selectors.sizeBtns.forEach(btn => { btn.addEventListener("click", function() { selectors.sizeBtns.forEach(b => b.classList.remove("active-size")); this.classList.add("active-size"); selectors.priceVariantRadios.forEach(r => r.checked = false); document.querySelectorAll("#custom-measurements-form input").forEach(input => input.value = ''); if (selectors.productPriceEl) { selectors.productPriceEl.dataset.priceNgn = selectors.productPriceEl.dataset.basePrice; updateAllPrices(document.querySelector('.currency-switcher a.active')?.dataset.currency || 'NGN'); } }); });
    ...
    selectors.modals.forEach(({ modal, openBtn, closeBtn, overlay }) => { if (openBtn) { openBtn.addEventListener("click", (e) => { e.preventDefault(); if (modal && modal.id === "custom-size-modal") { selectors.sizeBtns.forEach(b => b.classList.remove("active-size")); selectors.priceVariantRadios.forEach(r => r.checked = false); if (selectors.productPriceEl) { selectors.productPriceEl.dataset.priceNgn = selectors.productPriceEl.dataset.basePrice; updateAllPrices(document.querySelector('.currency-switcher a.active')?.dataset.currency || 'NGN'); } } toggleModal(modal, true); }); } if (closeBtn) { closeBtn.addEventListener("click", () => toggleModal(modal, false)); } if (overlay) { overlay.addEventListener("click", () => toggleModal(modal, false)); } });
    ...
    document.querySelectorAll('.currency-switcher a.currency-link').forEach(link => { link.addEventListener('click', function(e) { e.preventDefault(); const targetCurrency = this.dataset.currency; document.querySelectorAll('.currency-switcher a.currency-link').forEach(l => l.classList.remove('active')); this.classList.add('active'); updateAllPrices(targetCurrency); const expiryDate = new Date(); expiryDate.setDate(expiryDate.getDate() + 30); document.cookie = `user_currency=${targetCurrency}; expires=${expiryDate.toUTCString()}; path=/`; fetch('/currency', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ currency: targetCurrency }) }); }); });
