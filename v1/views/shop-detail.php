  ...
  <body class="bg-brand-bg font-sans text-brand-text">
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const updateCartDisplay = async () => {
            try {
                // **FIX**: The `update-cart` endpoint is called via POST, but with an empty body as it gets the token from the cookie.
                ...
                if (cartData && cartData.status === 'success') {
                    ...
                    if (cartData.items && cartData.items.length > 0) {
                            ...
                            const itemHtml = `
                                <div class="flex gap-4 py-4 border-b border-gray-200 last:border-b-0">
                                    ...
                                    <div class="flex-1 flex flex-col justify-between">
                                        ...
                                        <div class="flex justify-between items-center mt-2"><p class="text-sm font-medium">₦${parseFloat(item.total_price).toFixed(2)}</p><p class="text-xs text-brand-gray">Qty: ${item.quantity}</p></div>
                                    </div>
                        ...
                        // **FIX**: If the items array is empty, restore the empty cart message HTML.
                        const emptyCartHtml = `
