...
$thirty_days = time() + (86400 * 30); // 86400 seconds = 1 day
  ...
  <body class="bg-brand-bg font-sans text-brand-text">
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const selectors = {
            ...
            cartItemsContainer: document.getElementById("cart-items-container"), cartSubtotalEl: document.getElementById("cart-subtotal"), cartItemCountEl: document.getElementById("cart-item-count"), emptyCartMessage: document.getElementById("empty-cart-message"),
        ...
        const showToast = (text, type = "success") => {
            ...
            Toastify({ text, duration: 3000, close: true, gravity: "top", position: "right", style: { background: backgroundColor } }).showToast();
        };
        ...
        const toggleSidebar = (sidebar, shouldOpen) => {
            ...
            if (shouldOpen) {
                ...
                const isAnotherSidebarOpen = document.querySelector("#sidebar:not(.-translate-x-full), #cart-sidebar:not(.translate-x-full)");
                if (!isAnotherSidebarOpen) document.body.classList.remove("overflow-hidden");
            }
        };
        ...
        const toggleModal = (modal, shouldOpen) => {
            ...
            if (shouldOpen) {
                ...
                modal.classList.add("hidden"); if (!document.querySelector(".modal-container:not(.hidden)")) { document.body.classList.remove("overflow-hidden"); }
            }
        };
        ...
        // -- NEW: FUNCTION TO UPDATE CART DISPLAY --
        const updateCartDisplay = async () => {
            try {
                // Use a descriptive route like '/cart-contents' or the direct path to your new API file.
                ...
                if (cartData.items && cartData.items.length > 0) {
                    ...
                    cartData.items.forEach(item => {
                        ...
                        if (item.size_name) optionsHtml += ` / ${item.size_name}`;
                        else if (item.custom_size_details) optionsHtml += ' / Custom Size';
                            ...
                            <div class="flex gap-4 py-4 border-b border-gray-200 last:border-b-0">
                                ...
                                <div class="flex-1 flex flex-col justify-between">
                                    ...
                                    <div class="flex justify-between items-center mt-2">
                                        <p class="text-sm font-medium">₦${parseFloat(item.total_price).toFixed(2)}</p>
                    ...
                    selectors.emptyCartMessage.style.display = 'flex';
                }

                selectors.cartSubtotalEl.textContent = `₦${parseFloat(cartData.subtotal).toFixed(2)}`;
                selectors.cartItemCountEl.textContent = cartData.totalItemCount;
                selectors.cartItemCountEl.style.display = cartData.totalItemCount > 0 ? 'flex' : 'none';
        ...
        const handleAddToCart = async () => {
            ...
            else { showToast("Please select a color.", "error"); resetAddToCartButton(); return; }
            ...
            if (activeSizeBtn) { payload.sizeId = parseInt(activeSizeBtn.dataset.id); } 
                ...
                if (Object.keys(payload.customSizeDetails).length === 0) { showToast("Please select a size or provide custom measurements.", "error"); resetAddToCartButton(); return; }
            }

            try {
                // Use the direct path to your API file.
                ...
                if (response.ok && result.status === "success") { 
                    ...
                    await updateCartDisplay(); // Refresh the cart display
                    toggleSidebar(selectors.cartSidebar, true); // Open the cart
        ...
        if(selectors.openCartBtn) selectors.openCartBtn.addEventListener("click", async () => {
            await updateCartDisplay(); // Refresh cart before opening
        ...
        // -- NEW: Initial load of cart data for the top icon --
        updateCartDisplay();
    });
    </script>
  </body>
